  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button  type="primary" size="small" :disabled="scope.row.superAdmin" @click="$router.push({
          ...
          >
            <el-button :disabled="scope.row.superAdmin" slot="reference" class="ml10" size="small" type="danger">
              删除
            </el-button>
          </el-popconfirm>
        </template>
...
export default {
  ...
  methods: {
    onChange(val, id) {
      val
        ? adminuserEnable({ id }).then((res) => {
            this.fetchData();
          })
        : adminuserDisable({ id }).then((res) => {
            this.fetchData();
          });
    },
    onDelete(id) {
      adminuserDelete({ id }).then((res) => {
    ...
    fetchData() {
      ...
      getAdminUserList(this.page).then((res) => {
